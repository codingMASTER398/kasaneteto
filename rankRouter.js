...
router.post("/new", limiter, (req, res) => {
  const A =
    songsToChooseFrom[Math.floor(Math.random() * songsToChooseFrom.length)];
...
router.post(`/choose/:choice/:token`, (req, res) => {
  ...
  // Update percentages
  db[data.A.id].perc =
    (100 * db[data.A.id].pos) / (db[data.A.id].pos + db[data.A.id].neg);
  db[data.B.id].perc =
    (100 * db[data.B.id].pos) / (db[data.B.id].pos + db[data.B.id].neg);
